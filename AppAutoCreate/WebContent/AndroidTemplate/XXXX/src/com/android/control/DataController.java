...
import com.android.control.cache.CacheManager;
import com.android.control.cache.CachePolicy;
...
public class DataController {
	...
	public long cacheTimeOut = 0;  //default 0 使用默认的缓存时间
	...
	/**
	 * 直接从网上获取数据
	 * @param policy  ** 直接使用该方法从网上获取数据时 注意 POLICY_CACHE_AND_REFRESH 方式不会调用 onsuccess；
	 * @param callBack
	 */
	public void getDateFromNet(final NetCachePolicy policy,
		...
		net.send(httpMethod, url, params, new RequestCallBack<String>() {
			...
			public void onSuccess(ResponseInfo<String> responseInfo) {
				...
				if(debug)Log.i(tag, response.success+"||"+response.status);
				// 请求正确
				if (response.success) {
					switch (policy) {
					case POLICY_CACHE_AND_REFRESH:
